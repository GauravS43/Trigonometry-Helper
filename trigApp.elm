import Array
import Random
--RANDOMLY GENERATED QUESTIONS IN THE QUIZZES!!
...
drawButton h txt lightClr darkClr = group
  [ rect 45 h |> filled lightClr 
  , rect 45 h |> outlined (solid 1) darkClr 
  , text txt |> size 5 |> centered |> filled black  |> move (0,-2)
  ]

drawRButton h txt1 txt2 lightClr darkClr = group
  [ roundedRect 45 h 10 |> filled lightClr   
  , roundedRect 45 h 10 |> outlined (solid 1) darkClr 
  , text txt1 |> size 7 |> centered |> filled black  |> move (0,1)
  , text txt2 |> size 7 |> centered |> filled black  |> move (0,-6)
  ]
...
myShapes model =
    case model.state of
        MainMenu  ->
            ...
            , drawRButton 25 "Basic" "Trigonometry" lightYellow darkYellow |> move (-50, 0) |> notifyTap ToSOHCAHTOA
            , drawRButton 25 "Basic Trig" "Quiz" lightGreen darkGreen |> move (-50, -40) |> notifyTap ToSOHQuiz
            , drawRButton 25 "Sine + Cosine" "Visualizer" lightBlue blue |> move (0, -20) |> notifyTap ToSCViz
            , drawRButton 25 "Cosine & Sine" "Laws" (rgb 255 154 155) red |> move (50, 0) |> notifyTap ToSineCosine
            , drawRButton 25 "Cosine & Sine" "Laws Quiz" (rgb 213 139 232) darkPurple |> move (50, -40) |> notifyTap ToSCQUIZ
            ]
        SOHCAHTOAQuiz  ->
            [ drawSCT model
            ]
...
update msg model =
    case msg of
        ...
        ToSineCosine  ->
            case model.state of
                MainMenu  ->
                   ( { model | state = SineCosine, rightSide = 0, leftSide = 0, bottom = 0, topAngle = 0, rightAngle = 0, leftAngle = 0, errorMsg = " " }, Cmd.none )
                otherwise ->
                   ( model, Cmd.none )
        ToSCQUIZ  ->
          ( { model | state = SineCosineQuiz, scqScore = 0, scqLimit = 3, scqResult = "", scqClr = lightPurple}, scqCmdScreen model )
        ToSCViz  ->
            case model.state of
                MainMenu  ->
        ...
        ToSOHCAHTOA  ->
            case model.state of
                ...
                otherwise ->
                   ( model, Cmd.none )
        ToSOHQuiz  ->
          ( { model | state = SOHCAHTOAQuiz, score = 0, limit = 3, result = "", clr = lightGreen }, cmdScreen model )
        --SOHCAHTOA QUIZ CMDS
        RightAnswer ->
        ...
        Reset -> ( {model | topAngle = 0, leftAngle = 0, rightAngle = 0, leftSide = 0, rightSide = 0, bottom = 0, errorMsg = ""}, Cmd.none )
        ...
        BTCalculate -> ( {model | btErrorMsg = (if (((model.btLeftSide) > 0) && ((model.btRightSide) > 0) && ((model.btBottom) > 0) )
                                   ...
                                   , btLeftSide = (if model.btErrorMsg == "" then (if((model.btBottom < model.btRightSide) && (model.btRightSide > 0 && model.btBottom > 0))
                                                 then ((((toFloat (round (100*(sqrt((model.btRightSide * model.btRightSide)-(model.btBottom * model.btBottom))))))/100)))
                                                 else model.btLeftSide) else model.btLeftSide)
                                   
                                   , btRightSide = (if model.btErrorMsg == "" then ((if((model.btBottom > 0 && model.btLeftSide > 0))
                                                 then ((((toFloat (round (100*(sqrt((model.btLeftSide * model.btLeftSide)+(model.btBottom * model.btBottom))))))/100)))
                                                 else model.btRightSide)) else model.btRightSide)
                                                 
                                   , btBottom = (if model.btErrorMsg == "" then ((if((model.btLeftSide < model.btRightSide) && (model.btRightSide > 0 && model.btLeftSide > 0))
                                                 then ((((toFloat (round (100*(sqrt((model.btRightSide * model.btRightSide)-(model.btLeftSide * model.btLeftSide))))))/100)))
                                                 else model.btBottom)) else model.btBottom)
                                   , btTopAngle = (if model.btErrorMsg == "" then (findAngle model.btTopAngle model.btBottom model.btLeftSide model.btRightSide) else model.btTopAngle)
                                   , btRightAngle = (if model.btErrorMsg == "" then (findAngle model.btRightAngle model.btLeftSide model.btBottom model.btRightSide) else model.btRightAngle)
                                   }, Cmd.none)
...
drawFuncVis model = group
  ...
  , drawButton2 model.animationButton |> move (36, -100)
  , group [
        group[
            circle 10
  ...
  , drawValues |> scale 0.9 |> move (-44, -49) 
  ]

drawValues = group
  [ rect 110 28 |> outlined (solid 0.5) black |> move (0, -0.5)
  , text "Sin(0) = 0"  |> size 5 |> filled black |> move(-50,8)
  , text "Sin(π/6) = 1/2"  |> size 5 |> filled black |> move(-15,8)
  , text "Sin(π/4) = 1/√2"  |> size 5 |> filled black |> move(20,8)
  , text "Sin(π/3) = √3/2"  |> size 5 |> filled black |> move(-50,1)
  , text "Sin(π/2) = 1"  |> size 5 |> filled black |> move(-15,1)
  , text "Sin(π) = 0" |> size 5 |> filled black |> move(20,1)
  , text "Cos(0) = 1"  |> size 5 |> filled black |> move(-50,-6)
  , text "Cos(π/6) = √3/2" |> size 5 |> filled black |> move(-15,-6)
  , text "Cos(π/4) = 1/√2" |> size 5 |> filled black |> move(20,-6)
  , text "Cos(π/3) = 1/2" |> size 5 |> filled black |> move(-50,-13)
  , text "Cos(π/2) = 0"  |> size 5 |> filled black |> move(-15,-13)
  , text "Cos(π) = -1" |> size 5 |> filled black |> move(20,-13)
  ]
drawButton2 status =
  case status of
    Play ->
...
drawSC model = group
  ...
  , text "Cosine & Sine Laws" |> filled black |> move(-86,45)
...
drawCSTriangle model = group
  ...
  , buttons 1 1 |> move(32,23.5) -- rightSide
  , buttons 2 1 |> move(-32,23) -- leftSide
  , buttons 3 1 |> move(7,-11.5) -- bottom
  , buttons 4 1 |> move(4,28.5) -- topAngle
  , buttons 5 1 |> move(-18,2) -- leftAngle
  , buttons 6 1 |> move(18,2) -- rightAngle   
  ...
  , text (model.errorMsg) |> filled red |> scale 0.35 |> move(-80,32)
...
buttons idx ver = group
  [
  triangle 2
    |> filled black
    |> rotate (degrees 90)
    |> move(-40,-12)
    |> notifyTap (if ver == 1 then (Increase idx) else (BTIncrease idx))
    ,
    triangle 2
    |> filled black
    |> rotate (degrees 90)
    |> scaleY -1
    |> move(-40,-15)
    |> notifyTap (if ver == 1 then (Decrease idx) else (BTDecrease idx))
  ]
...
drawBTTriangle model = group
  ...
  , buttons 1 2 |> move(-1,23.5) -- rightSide
  , buttons 2 2 |> move(-52,23) -- leftSide
  , buttons 3 2 |> move(-6,-11.5) -- bottom
  ...
  , text (String.append (String.fromFloat model.btTopAngle) " °") |> filled black |> scale 0.35 |> move(-85,26)
  ...
  , text (model.btErrorMsg) |> filled red |> scale 0.35 |> move(-80,31)
  , rect 4 4 |> outlined (solid 0.75) darkYellow |> move (-73.25, -13)
  ]
